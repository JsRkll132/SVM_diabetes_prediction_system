...
alpha, b, K_train = train_svm(X_train, Y_train, C=1.0, gamma=0.5)

# 5. Evaluar el modelo en el conjunto de entrenamiento
train_predictions = predict(X_train, alpha, Y_train, X_train, b, gamma=0.5)
train_accuracy = accuracy_score(Y_train, train_predictions)
print(f"Accuracy on training data (RBF kernel): {train_accuracy * 100:.2f}%")

# Evaluar el modelo en el conjunto de prueba
test_predictions = predict(X_test, alpha, Y_train, X_train, b, gamma=0.5)
...
prediction = predict(std_data, alpha, Y_train, X_train, b, gamma=0.5)
